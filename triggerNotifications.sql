...
BEGIN
	...
	select ID into @vChargeback from chargebacks order by `ID` desc limit 1;
	insert into druporta_tss_data.notifications(email,MID, name,block, notified, message, url, chargebackID) values(@vEmail, @vMID, @vName, @vBlock, 0,'', '', @vChargeback );
END $$
DELIMITER ;
